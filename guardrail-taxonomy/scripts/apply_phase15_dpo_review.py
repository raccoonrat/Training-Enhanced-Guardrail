#!/usr/bin/env python3
"""Apply human-review decisions to Phase 1.5 DPO preference pairs."""
...
DEFAULT_OUTPUT = ROOT / "training" / "phase15-dpo-preference-reviewed.jsonl"
DEFAULT_DECISIONS = ROOT / "evaluation" / "review" / "phase15-dpo-review-decisions.jsonl"
DEFAULT_REPORT = ROOT / "evaluation" / "review" / "phase15-dpo-review-apply-report.json"
...
def _load_decisions(path: Path) -> dict[str, dict]:
    ...
    for record in _load_jsonl(path):
        ...
        if not sample_id:
            raise ValueError(f"{path}: DPO review decision missing sample_id")
        if sample_id in decisions:
            raise ValueError(f"{path}: duplicate DPO review decision for {sample_id}")
...
def apply_reviews(records: list[dict], decisions: dict[str, dict]) -> tuple[list[dict], dict]:
    ...
    for record in records:
        ...
        quality = record.setdefault("quality", {})
        quality.setdefault("source", "phase15_dpo_bootstrap")
        quality.setdefault("difficulty", "medium")

        if review_decision == APPROVE and reviewer and reviewed_at and checklist_passed:
...
def main() -> int:
    ...
    parser.add_argument("--in-place", action="store_true", help="Overwrite --dpo after validation.")
    parser.add_argument("--dry-run", action="store_true", help="Validate and report without writing output.")
    args = parser.parse_args()

    records = _load_jsonl(args.dpo)
    ...
    report.update(
        {
            ...
            "output": str(args.dpo if args.in_place else args.output),
    ...
    if not args.dry_run:
        target = args.dpo if args.in_place else args.output
        _write_jsonl(target, reviewed_records)

    args.report.parent.mkdir(parents=True, exist_ok=True)
    args.report.write_text(json.dumps(report, indent=2, ensure_ascii=False) + "\n", encoding="utf-8")

    print(f"DPO review decisions applied: {len(decisions)}")
