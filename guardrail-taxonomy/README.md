...
原始 bootstrap 记录用于 Phase 1.5 管线演练和人工审核 bootstrap；当前训练就绪版本已经完成 SFT 一审/二审与 DPO 人审：

- SFT：`training/phase15-sft-reviewed-second.jsonl`
- DPO：`training/phase15-dpo-preference-reviewed.jsonl`
- 联合严格审计：`evaluation/review/phase15-sft-dpo-reviewed-quality-report.json`
- 资产冻结清单：`evaluation/phase15-training-freeze.json`

这些 reviewed 资产仍是 bootstrap 规模（104 SFT + 52 DPO），不替代最终 5K–10K 高质量数据集。
...
当前可执行审核闭环覆盖 SFT 与 DPO：SFT 已完成一审 + 独立二审，DPO preference pairs 已完成人工审核；两者均接入 `audit_phase15_assets.py` 的严格门槛。
...
6. SFT 一审 104/104、独立二审 80/80、DPO 人审 52/52
7. SFT+DPO bootstrap 训练资产冻结：`evaluation/phase15-training-freeze.json`
8. Phase 1.5 扩量计划与机器配置：
   - `evaluation/phase15-scale-plan.md`
   - `evaluation/phase15-scale-config.json`
9. 第一批扩量 dry-run count planner：
   - `scripts/generate_phase15_scaled_assets.py --dry-run`
   - `evaluation/phase15-scale-dry-run-report.json`
10. 第一批 expanded assets 已生成并通过基础审计：
    - SFT：`training/phase15-sft-scaled-5200.jsonl`
    - DPO：`training/phase15-dpo-scaled-2600.jsonl`
    - manifest：`evaluation/phase15-scale-manifest.json`
    - audit：`evaluation/review/phase15-scale-quality-report.json`
11. LLM-as-judge 二级审核队列与应用脚本已就绪：
    - queue：`evaluation/review/phase15-llm-judge-queue.jsonl`
    - decisions：`evaluation/review/phase15-llm-judge-decisions.jsonl`
    - report：`evaluation/review/phase15-llm-judge-queue-report.json`
    - apply：`scripts/apply_phase15_llm_judge_review.py`

待推进：

1. 填写 `phase15-llm-judge-decisions.jsonl` 的 `judge_model` / `judged_at` / verdict
2. 应用 LLM judge 结果并导出 `*-judged.jsonl`
3. 对 `needs_changes` / `reject` / `escalate_to_human` 与 critical 样本执行必要人工复核
4. 通过复核后生成 reviewed expanded assets 与冻结清单
5. 将 runner 接入 CI/CD release gate
